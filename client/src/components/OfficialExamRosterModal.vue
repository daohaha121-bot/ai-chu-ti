<template>
  ...
  >
    <div class="space-y-4">
      <!-- 顶部操作提示与导出按钮 -->
      <div class="flex items-center justify-between bg-blue-50/80 border border-blue-100 p-3.5 rounded-xl">
        <div class="flex items-center gap-2 text-blue-900 text-sm">
          <el-icon class="text-blue-600 text-base"><InfoFilled /></el-icon>
          <span>已根据真实考试规范自动生成<strong>《官方考核成绩总表》</strong>，点击右侧按钮即可一键保存超清长图。</span>
        </div>
      ...
      <!-- 预览可滚动视窗 -->
      <div class="max-h-[70vh] overflow-y-auto overflow-x-hidden p-3 bg-gray-100/80 rounded-xl flex justify-center border border-gray-200">
        <!-- 专为 html2canvas 独立渲染的高清画卷主体 (不含任何组件库 fixed 冲突结构) -->
        <div
          id="official-exam-roster-capture-area"
          class="w-[820px] bg-white p-8 rounded-xl shadow-md border border-gray-200/90 text-slate-800 space-y-6 relative"
        ...
        >
          ...
          <div class="border-b-2 border-slate-900 pb-5 text-center relative space-y-2">
            ...
            <div class="flex items-center justify-between text-[11px] text-slate-500 pt-1 px-1">
              <span class="font-mono">存证编号: ARCHIVE-{{ (exam?.id || 'EXAM').slice(0, 8).toUpperCase() }}-{{ archiveDateCode }}</span>
          ...
          <!-- 考生全量花名册与成绩明细表格 (纯原生 HTML 表格，保证 html2canvas 100% 渲染成功) -->
            ...
            <table class="w-full text-left border-collapse" style="table-layout: fixed;">
              <thead>
                <tr class="bg-slate-900 text-white text-xs">
                  <th class="py-2.5 px-3 font-semibold text-center" style="width: 50px;">序号</th>
                  <th class="py-2.5 px-3 font-semibold" style="width: 100px;">考生姓名</th>
                  <th class="py-2.5 px-3 font-semibold" style="width: 120px;">学号/工号</th>
                  <th class="py-2.5 px-3 font-semibold" style="width: 120px;">部门/班级</th>
                  <th class="py-2.5 px-3 font-semibold text-center" style="width: 85px;">考核得分</th>
                  <th class="py-2.5 px-3 font-semibold text-center" style="width: 85px;">评定结果</th>
                  <th class="py-2.5 px-3 font-semibold text-center" style="width: 100px;">考场纪律</th>
                  <th class="py-2.5 px-3 font-semibold text-right" style="width: 140px;">交卷时间</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-200 text-xs text-slate-700">
                ...
                >
                  ...
                  <td class="py-2.5 px-3 text-center font-bold text-sm font-mono" :class="sub.isPassed ? 'text-emerald-600' : 'text-rose-600'">
                    {{ sub.score }}分
                  </td>
                  ...
                  <td class="py-2.5 px-3 text-right font-mono text-[11px] text-slate-500 truncate">
          ...
          <!-- 底部签署栏与权威官方防伪红章 -->
          <div class="pt-4 border-t border-slate-200 flex items-end justify-between relative min-h-[110px]">
            <!-- 左侧合规落款 -->
            ...
            <!-- 右侧官方红色双圈权威印章 (纯矢量 CSS 绘制，100% 避免跨域且效果绝佳) -->
